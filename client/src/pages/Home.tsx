      ...
      <div className="flex min-h-screen">
        ...
        <main className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1500px] space-y-6 p-5 md:p-8">
            ...
            {active === "requests" && <RequestsView requests={requests.data ?? []} rooms={rooms} items={items} isAdmin={isAdmin} currentUserId={user?.id} todayRoomLocks={todayRoomLocks.data ?? []} selectedRoom={selectedRoom} selectedRoomName={selectedRoomName} setSelectedRoom={setSelectedRoom} lines={requestLines} setLines={setRequestLines} total={requestTotal} onCreate={(input: any) => createRequest.mutate(input)} onVerify={(input: any) => verifyRequest.mutate(input)} onDeliver={(id: number) => deliverRequest.mutate({ requestId: id })} onReceive={(id: number) => receiveRequest.mutate({ requestId: id })} busy={createRequest.isPending || verifyRequest.isPending || deliverRequest.isPending} />}
...
function StockView({ stock, isAdmin, items, warehouses, onCreateItem, busy, onImport, importBusy }: any) {
  ...
  return <Card className="border-slate-200/80 shadow-sm">
    <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
      <div><CardTitle>Saldo stok</CardTitle><p className="mt-1 text-sm text-slate-500">Saldo dihitung dari seluruh pergerakan barang.</p></div>
...
function StockOpnameView({ stock, items, rooms, onSubmit, busy }: any) {
  ...
  function submit() {
    ...
    onSubmit({ itemId: Number(form.itemId), roomId: form.roomId ? Number(form.roomId) : null, adjustmentType: difference > 0 ? "add" : "subtract", quantity: Math.abs(difference), physicalQty, reasonType: "stocktake", reason: form.reason.trim(), incidentDate: new Date(form.incidentDate) });
  }
  return <div className="grid gap-6 xl:grid-cols-[.9fr_1.1fr]">
    <Card className="border-slate-200/80 shadow-sm"><CardHeader><CardTitle>Stock Opname</CardTitle><p className="mt-1 text-sm text-slate-500">Stok sistem dibandingkan langsung dengan hasil hitung fisik.</p></CardHeader><CardContent className="space-y-4">
      ...
      <Field label="Ruangan terkait (opsional)"><select className="h-10 w-full rounded-md border border-input bg-background px-3 text-sm" value={form.roomId} onChange={(e) => setForm({ ...form, roomId: e.target.value })}><option value="">Umum / gudang</option>{rooms.map((room: any) => <option key={room.id} value={room.id}>{room.name}</option>)}</select></Field>
      ...
      <p className="text-xs leading-5 text-slate-400">Hasil fisik menjadi sumber penyesuaian akhir. Jika selisih 0, sistem tidak membuat transaksi.</p>
    </CardContent></Card>
    <Card className="border-slate-200/80 shadow-sm"><CardHeader><CardTitle>Contoh perhitungan</CardTitle></CardHeader><CardContent className="space-y-3 text-sm">
      ...
      <div className="rounded-xl border border-slate-200 p-4"><p className="font-medium">Audit</p><p className="mt-1 text-slate-500">Audit menyimpan stok sistem, stok fisik, selisih, dan stok akhir.</p></div>
    </CardContent></Card>
  </div>;
}
